...
// SUBMIT POST / VALIDATE INPUT
function submitPost() {
  // first get form and value data
  ...
  var data = {
    // if values are the same just put once instead title:title,
    title: title,
    body: body
    ...
    // Check for ID....if id is equal to nothing
    if (id === '') {
      // Create Post
      // Create Post Request..... url and data... gives us a promise
      _http.http.post('http://localhost:3000/posts', data).then(function (data) {
        _ui.ui.showAlert('Post added', 'alert alert-success');
        _ui.ui.clearFields();
        // after adding post we get to see
        getPosts();
      }).catch(function (err) {
        return console.log(err);
      });
...
function deletePost(e) {
  e.preventDefault();
  // use event propagation cause theres lots of delete btns
...
function enableEdit(e) {
  ...
  // were doing event deligation so we do if statement
  if (e.target.parentElement.classList.contains('edit')) {
    // if it is edit, we wanna get post info through target
    // id
    var id = e.target.parentElement.dataset.id;
    // title
    ...
    // use to see what we're selecting
...
var UI = function () {
  ...
  _createClass(UI, [{
    ...
    value: function showAlert(message, className) {
      ...
      // Add classes    we get the class name from className var
      div.className = className;

      // Add text/append and create textNode with the msg beign passed
      div.appendChild(document.createTextNode(message));

      //             Next Insert into dom
      // GET Parent
      ...
      container.insertBefore(div, posts); // insert div before posts


      // Timeout make it go away after 3 sec
      setTimeout(function () {
        _this.clearAlert();
      }, 3000);
    }
    ...
    value: function clearAlert() {
      ...
      // tets if theres an alert if there is we remove
      if (currentAlert) {
        currentAlert.remove();
      }
    }
    ...
    value: function clearFields() {
      // get title input and set value to nothing 
      this.titleInput.value = '';
      // get body input and set value to nothing 
    ...
    // create change form state function

  }, {
    key: 'changeFormState',
    value: function changeFormState(type) {
      // change text to update post, change color
      if (type === 'edit') {
        ...
        // give created btn a class
        button.className = 'post-cancel btn btn-light btn-block';
        // give newly created button some text 
